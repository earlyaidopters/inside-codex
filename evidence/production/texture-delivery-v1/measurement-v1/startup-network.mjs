...
export async function startupNetwork({bitsPerSecond=25_000_000,latencyMs=50,upstream='http://127.0.0.1:43211'}={}){
 ...
 function pump(){if(timer||!queue.length||closed)return;const job=queue.shift();const at=Math.max(performance.now(),nextAt,job.eligible);nextAt=at+job.bytes.length*8000/bitsPerSecond;
  timer=setTimeout(()=>{timer=undefined;if(!job.response.destroyed){if(!job.response.headersSent){job.response.writeHead(job.row.status,job.headers);job.row.headerBytes=Buffer.byteLength(job.response._header??'');}job.response.write(job.bytes);job.row.bodyBytes+=job.bytes.length;if(job.last){job.response.end();job.row.finishedAt=performance.now()-start;}}pump();},Math.max(0,nextAt-performance.now()));
 }
 const server=http.createServer((request,response)=>{
  ...
  const target=new URL(request.url,upstream);const incoming=http.request(target,{method:request.method,headers:{...request.headers,host:target.host}},res=>{const buffers=[];res.on('data',b=>buffers.push(b));res.on('end',()=>{const body=Buffer.concat(buffers);row.status=res.statusCode;row.upstreamBodyBytes=body.length;row.headers=res.headers;const headers={...res.headers};delete headers['transfer-encoding'];delete headers.connection;headers['content-length']=body.length;const eligible=performance.now()+latencyMs;
   if(!body.length){response.writeHead(row.status,headers);response.end();row.headerBytes=Buffer.byteLength(response._header??'');row.finishedAt=performance.now()-start;return;}
