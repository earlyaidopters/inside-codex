import {guideSightPoints,visibilityIndex,chooseTourCamera,orbitPosition} from './tour-camera.mjs';
...
export async function createWorld(canvas:HTMLCanvasElement,onSelect:(i:number)=>void,onChoose:(id:string)=>void,onInspect:(id:string)=>void,onModelAction:(action:string,id:string)=>void,onPermissionAction:(action:string)=>void,onReviewAction:(id:string)=>void,onSteeringAction:(id:string)=>void,onToolAction:(id:string)=>void,onAutomationAction:(id:string)=>void,onHandoffAction:(id:string)=>void,onCapstoneAction:(id:string)=>void){
 ...
 const ring=(name:string,pos:number[],diam:number,tube:number,m:PBRMaterial)=>{const o=MeshBuilder.CreateTorus(name,{diameter:diam,thickness:tube,tessellation:96},scene);o.position=new Vector3(...pos);o.material=m;return o;};
 ...
 const architecture=await SceneLoader.ImportMeshAsync('','/assets/headquarters/','architecture.glb',scene);
 ...
 const navigationResponse=await fetch('/assets/headquarters/navigation.json');if(!navigationResponse.ok)throw new Error('Camera geometry manifest missing');const navigation=await navigationResponse.json();const visibility=visibilityIndex(navigation.boxes);const cameraAudit=new URLSearchParams(location.search).has('cameraAudit');const baselineCamera=(import.meta as any).env.DEV&&new URLSearchParams(location.search).get('tourCamera')==='baseline';
 ...
 const pip=new DefaultRenderingPipeline('Finishing',true,scene,[camera]);pip.fxaaEnabled=true;pip.samples=4;pip.bloomEnabled=true;pip.bloomThreshold=.94;pip.bloomWeight=.12;pip.bloomKernel=48;
 const result=await SceneLoader.ImportMeshAsync('','/assets/','codex-mascot.glb',scene);
 ...
 let cameraChoice:any=null,cameraUnresolved=0,cameraAdjustments=0;
 let time=0,paused=false,reduced=false,quality='high',active=-1,transition=0,layout='welcome';
 ...
 const tourPose=(progress:number)=>{const t=smooth(progress),position=atRoute(t),baseTarget=position.add(Vector3.Lerp(fromTarget.subtract(fromPos),toTarget.subtract(toPos),t));const transit=travelPoints.length>2?smooth(progress/.18)*smooth((1-progress)/.18):0;const target=Vector3.Lerp(baseTarget,position.add(new Vector3(0,2.3,-1.6)),transit);return {position,target:target.asArray(),alpha:(fromAlpha+(toAlpha-fromAlpha)*t)*(1-transit)+1.55*transit,beta:(fromBeta+(1.31-fromBeta)*t)*(1-transit)+.62*transit,radius:(fromRadius+(toRadius-fromRadius)*t)*(1-transit)+4.6*transit};};
 ...
 const travel=(index:number)=>{
  ...
  cameraChoice={target:camera.target.asArray(),alpha:camera.alpha,beta:camera.beta,radius:camera.radius};transition=reduced?1:.0001;gesture('Travel');
 };
 scene.onBeforeRenderObservable.add(()=>{
  ...
  if(transition>0&&!paused){transition=Math.min(1,transition+dt/travelDuration);const t=transition*transition*(3-2*transition);mascot.position=atRoute(t);const baselineTarget=mascot.position.add(Vector3.Lerp(fromTarget.subtract(fromPos),toTarget.subtract(toPos),t));const planned=tourPose(transition);const desired=baselineCamera?{target:baselineTarget.asArray(),alpha:fromAlpha+(toAlpha-fromAlpha)*t,beta:fromBeta+(1.31-fromBeta)*t,radius:fromRadius+(toRadius-fromRadius)*t}:planned;const target=Vector3.FromArray(desired.target);
   const scale=Math.max(mascot.scaling.x,innerWidth<=700&&active>=0&&layout==='mission'?.76:active<0?1.15:.86),lift=presentation.position.y;const points=guideSightPoints(mascot.position.asArray(),scale,lift);
   const ahead=Math.min(1,transition+.55/travelDuration),futurePose=tourPose(ahead),futurePos=futurePose.position,futureTarget=Vector3.FromArray(futurePose.target);
   const choice=baselineCamera?{...desired,adjusted:false}:chooseTourCamera(visibility,desired,points,{target:futureTarget.asArray(),points:guideSightPoints(futurePos.asArray(),scale,lift)},cameraChoice);
   let pose=choice;if(!baselineCamera&&cameraChoice&&!reduced&&transition<1){const blend=Math.min(1,dt*6);const smooth={...choice,alpha:cameraChoice.alpha+(choice.alpha-cameraChoice.alpha)*blend,beta:cameraChoice.beta+(choice.beta-cameraChoice.beta)*blend,radius:cameraChoice.radius+(choice.radius-cameraChoice.radius)*blend};if(!visibility.blocked(orbitPosition(target.asArray(),smooth.alpha,smooth.beta,smooth.radius),points,.04))pose=smooth;}
   cameraChoice=pose;if(choice.adjusted)cameraAdjustments++;if(choice.unresolved)cameraUnresolved++;camera.setTarget(target,false,true,true);camera.alpha=pose.alpha;camera.beta=pose.beta;camera.radius=pose.radius;if(transition===1){transition=0;gesture(active<0?'Return':'Greet');}}
  if(!paused){
 ...
 function cameraVisibility(){const points=guideSightPoints(mascot.position.asArray(),mascot.scaling.x,presentation.position.y),position=camera.globalPosition.asArray();const proxy=points.map(p=>visibility.firstHit(position,p,.02));const exact=cameraAudit?points.map(p=>{const end=Vector3.FromArray(p),delta=end.subtract(camera.globalPosition),ray=new Ray(camera.globalPosition,delta.normalize(),Vector3.Distance(camera.globalPosition,end)-.02);const hit=scene.pickWithRay(ray,m=>architecture.meshes.includes(m)&&!m.name.includes('floor')&&!m.name.includes('exterior'));return hit?.hit?{mesh:hit.pickedMesh?.name,distance:hit.distance}:null;}):null;return {mode:baselineCamera?'baseline':'candidate',position,points,proxy,exact,adjustments:cameraAdjustments,unresolved:cameraUnresolved};}
 function applyRenderQuality(v:string){quality=v;engine.setHardwareScalingLevel(v==='balanced'?1.5:1/Math.min(2,Math.max(1,window.devicePixelRatio||1)));pip.bloomEnabled=v==='high';pip.samples=v==='high'?4:1;}
 const resize=()=>{applyRenderQuality(quality);fromTarget=camera.target.clone();fromPos=mascot.position.clone();fromRadius=camera.radius;fromAlpha=camera.alpha;fromBeta=camera.beta;responsiveCamera();transition=1;};
