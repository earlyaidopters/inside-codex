...
async function loadArchitecture(scene:Scene){
 ...
 const response=await fetch('/assets/headquarters/streamed/architecture.glb.gz');
...
export async function createWorld(canvas:HTMLCanvasElement,onSelect:(i:number)=>void,onChoose:(id:string)=>void,onInspect:(id:string)=>void,onModelAction:(action:string,id:string)=>void,onPermissionAction:(action:string)=>void,onReviewAction:(id:string)=>void,onSteeringAction:(id:string)=>void,onToolAction:(id:string)=>void,onAutomationAction:(id:string)=>void,onHandoffAction:(id:string)=>void,onCapstoneAction:(id:string)=>void,initialQuality:MascotDetail='high',onQualityStatus:(message:string)=>void=()=>{},onExhibitStatus:(status:ExhibitStatus)=>void=()=>{}){
 ...
 await architectureDetail.select(initialQuality);
 const lightmaps:Record<string,Texture>={};
 const architectureTextures=await installArchitectureTextures(scene,architecture.meshes,lightmaps);
 if(architectureTextures.mode==='original')await installOriginalArchitectureTextures(scene,architecture.meshes,lightmaps);
