...
const out=process.env.EVIDENCE_DIR??'../evidence/production/release-orbit-v1';await fs.mkdir(out,{recursive:true});
...
try{await p.goto(process.env.TEST_URL??'http://127.0.0.1:43211/?cameraAudit');await p.waitForFunction(()=>window.__insideCodex?.state().ready);await p.getByRole('button',{name:'Step inside',exact:false}).click();
