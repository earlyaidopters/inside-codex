...
assert(out,'Run the audio-soak-release game-dev scenario.');
...
const inputs=await files('dist-camera-review'),buildId=hash(Buffer.from(JSON.stringify(inputs)));
...
try{
 await page.goto('http://127.0.0.1:43212/?renderAudit&audioAudit');
 ...
 assert(report.observation.voiceSamples>0);assert(report.observation.audibleSamples>0);assert.deepEqual(await files('dist-camera-review'),inputs,'Build changed during soak');
