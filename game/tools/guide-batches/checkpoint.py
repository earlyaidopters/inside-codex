...
root=Path(__file__).resolve().parents[3]; stage=root/'evidence/production/guide-batches-v1'
read=lambda p:json.loads(p.read_text())
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
a=read(stage/'assessment.json'); assert a['pass']
base=read(root/'evidence/production/checkpoint-handoff-summary-v1.json'); assert sha(root/base['archive'])==base['sha256']
archive=root/'releases/checkpoints/guide-batches-v1-source.zip'; assert not archive.exists()
...
paths += [root/p for p in ['game/.game-dev/goals/skinned-guide-submissions.json','production/STATUS.md','production/DEFECTS.md','production/GUIDE-SKIN-BATCHES.md','production/RENDER-COST-INVESTIGATION.md']]
for folder in [stage,root/'game/tests/guide-batches',root/'game/tools/guide-batches',root/'production/sources/release-2026-09-10']:paths += [p for p in folder.rglob('*') if p.is_file() and '__pycache__' not in p.parts]
rows=[]
...
(root/'evidence/production/checkpoint-guide-batches-v1.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt))
