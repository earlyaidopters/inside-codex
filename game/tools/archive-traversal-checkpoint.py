...
root=Path('..').resolve(); evidence=root/'evidence/production'; stage=evidence/'traversal-v1'
...
def sha(p):
 ...
 return h.hexdigest()
a=read(stage/'assessment.json');assert a['evidenceVerified'] and not a['fullPerformanceAcceptance'];assert len(a['runs'])==8
for p in ['marker-views/comparison.json','marker-journey/report.json','counter-verification/report.json']:assert read(stage/p)['pass']
assert read(stage/'quality-memory/report.json')['observationCompleted']
assert 'pass 71' in (stage/'unit-marker-v1.log').read_text() and 'fail 0' in (stage/'unit-marker-v1.log').read_text()
...
manifest=evidence/'dist-traversal-v1-manifest.json';manifest.write_text(json.dumps({'buildId':a['candidateBuildId'],'files':files},indent=2)+'\n')
previous=root/'releases/checkpoints/startup-delivery-v1-tested.zip'
with zipfile.ZipFile(previous) as z:names={n for n in z.namelist() if not n.startswith('game/dist/') and (root/n).is_file()}
for folder in ['game/src','game/public','game/dist','game/tests','game/tools','production','evidence/production/traversal-v1','game/.game-dev/goals']:
...
archive=root/'releases/checkpoints/traversal-v1-tested.zip'
