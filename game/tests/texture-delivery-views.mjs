...
const b=process.env.BROWSER==='firefox'?await firefox.launch({headless:true}):await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});const rows=[],errors=[];
try{
 ...
 assert.deepEqual(errors,[]);await fs.writeFile(`${out}/${phase}-${mode}.json`,JSON.stringify({pass:true,browser:b.version(),rows,errors},null,2));console.log(`${phase}: ${rows.length} views captured`);
