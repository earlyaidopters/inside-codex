...
try{
 ...
 assert.equal(world.architecture.detail.triangles,223458);assert(world.renderAudit.lastFrame.mainDrawCalls>0);assert(world.renderAudit.meshes.some(m=>m.name.startsWith('Batch:')));
 ...
 assert.deepEqual(errors,[]);await p.screenshot({path:'../evidence/production/static-parts-v1/no-uint.png'});
 await fs.writeFile('../evidence/production/static-parts-v1/no-uint.json',JSON.stringify({pass:true,world,errors,scope:'Chrome capability fault fixture, not physical WebGL 1 hardware'},null,2));
