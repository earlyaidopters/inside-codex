...
const state=p=>p.evaluate(()=>window.__insideCodex.state().world);
...
try{
 ...
 await fs.writeFile(`${out}/report.json`,JSON.stringify({pass:true,browser:browser.version(),scope:'Desktop Chrome and portrait viewport emulation, actual UI controls, delayed/503 network fixtures. Does not establish physical-device performance or long-run stability.',checks,errors,receipts},null,2));console.log(JSON.stringify({pass:true,checks:checks.length,errors}));
