...
try{
 ...
 await p.getByRole('button',{name:/Step inside|Continue exploring/}).click();await p.setViewportSize({width:390,height:844});await p.waitForFunction(()=>{const w=window.__insideCodex.state().world;return w.transition===0&&w.renderSize.cssWidth===390&&w.renderSize.width===390;});const portrait=await state(p);assert.equal(portrait.renderSize.width,portrait.renderSize.cssWidth);await p.screenshot({path:`${out}/balanced-portrait.png`});
