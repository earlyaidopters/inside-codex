...
const phase=process.argv[2];assert(['baseline','candidate'].includes(phase));const out=process.env.EVIDENCE_DIR??'../evidence/production/traversal-v1/marker-views';await fs.mkdir(out,{recursive:true});
...
try{
 for(const quality of ['high','balanced']){
  ...
  for(const [name,index] of [['arrival',null],['control',0],['tasks',4],['tools',8],['handoff',11]]){
   ...
   const mask=await p.addStyleTag({content:'#app{visibility:hidden!important}'});const filename=`${phase}-${quality}-${name}.png`;await p.screenshot({path:`${out}/${filename}`});await mask.evaluate(el=>el.remove());
 ...
 assert.deepEqual(errors,[]);await fs.writeFile(`${out}/${phase}.json`,JSON.stringify({pass:true,browser:b.version(),rows,errors},null,2));console.log(`${phase}: ${rows.length} views captured`);
