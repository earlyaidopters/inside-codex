import {buildWingBackdrop} from './wing-backdrop';
import {sampleWorldTour,tourDuration} from './world-tour.mjs';
import {createCameraClearance} from './camera-clearance';
import clearanceData from './camera-clearance-data.json';
...
export async function createWorld(canvas:HTMLCanvasElement,onSelect:(i:number)=>void,onChoose:(id:string)=>void,onInspect:(id:string)=>void,onModelAction:(action:string,id:string)=>void,onPermissionAction:(action:string)=>void,onReviewAction:(id:string)=>void,onSteeringAction:(id:string)=>void,onToolAction:(id:string)=>void,onAutomationAction:(id:string)=>void,onHandoffAction:(id:string)=>void,onCapstoneAction:(id:string)=>void,initialQuality:MascotDetail='high',onQualityStatus:(message:string)=>void=()=>{},onExhibitStatus:(status:ExhibitStatus)=>void=()=>{},startup:StartupMonitor=createStartupMonitor(),onQualityAdvice:(value:QualityAdvice)=>void=()=>{},onBrowserAction:(id:string)=>void=()=>{}){
 ...
 const wingColors=[violet,mint,warm];
 for(let wing=0;wing<3;wing++){
  const base=(wing-1)*16;
  
  buildWingBackdrop(scene,wing,base);
  for(let j=0;j<4;j++){
   ...
   for(let q=0;q<(i===0||i===1||i===2||i===3||i===5||i===6||i===7||i===8||i===9||i===10||i===11?0:3);q++){
    let ob:Mesh;
 ...
 const browserExhibit=residency.register({id:'browser',station:9,center:stations[9].position.add(new Vector3(0,2.6,0)),modulePath:'src/browser-exhibit.ts',exportName:'buildBrowserExhibit',load:()=>import('./browser-exhibit'),make:factory=>factory(scene,stations[9].position,onBrowserAction)});
 ...
 const cameraClearance=createCameraClearance(camera,clearanceData.boxes);
 let depthCamera:any=null;
 let cinematic:{elapsed:number;onDone:()=>void}|null=null;
 ...
 const responsiveCamera=()=>{
  if(depthCamera){camera.viewport=innerWidth<=700?new Viewport(0,.56,1,.44):new Viewport(0,0,.62,1);engine.resize();return;}
  const small=innerWidth<=700,compact=innerWidth>700&&innerHeight<=500;
  camera.viewport=layout==='tour'?new Viewport(0,0,1,1):compact?(layout==='mission'?new Viewport(0,0,.49,1):new Viewport(.49,0,.51,1)):small?(layout==='mission'?new Viewport(0,.48,1,.52):new Viewport(0,0,1,.56)):layout==='mission'?new Viewport(0,0,(innerWidth-(innerWidth<=1100?370:innerWidth>=1600?445:400)-innerWidth*.055)/innerWidth,1):new Viewport(0,0,1,1);
 ...
 const travel=(index:number)=>{
  cameraClearance.reset();camera.inertialAlphaOffset=0;camera.inertialBetaOffset=0;camera.inertialRadiusOffset=0;const wasTravelling=transition>0;const previousIndex=active;active=index;residency.focus(index);reminderGiven=false;fromTarget=camera.target.clone();fromPos=mascot.position.clone();fromRadius=camera.radius;fromAlpha=camera.alpha;fromBeta=camera.beta;
 ...
 const openContextDepth=(station=1)=>{if(depthCamera)return;depthCamera={station,target:camera.target.clone(),alpha:camera.alpha,beta:camera.beta,radius:camera.radius,position:mascot.position.clone(),scale:mascot.scaling.x,lift:presentation.position.y,transition,layout,clip:clip?.name,width:innerWidth,height:innerHeight,toTarget:toTarget.clone(),toRadius,toAlpha};if(station===9){toolExhibit.setEnabled(false);automationExhibit.setEnabled(false);}transition=0;layout='depth';camera.detachControl();camera.inertialAlphaOffset=0;camera.inertialBetaOffset=0;camera.inertialRadiusOffset=0;responsiveCamera();gesture('Point');};
 const closeContextDepth=()=>{if(!depthCamera)return;const saved=depthCamera;depthCamera=null;if(saved.station===9){toolExhibit.setEnabled(true);automationExhibit.setEnabled(true);}layout=saved.layout;responsiveCamera();toTarget=saved.toTarget;toRadius=saved.toRadius;toAlpha=saved.toAlpha;camera.setTarget(saved.target,false,true,true);camera.alpha=saved.alpha;camera.beta=saved.beta;camera.radius=saved.radius;mascot.position.copyFrom(saved.position);mascot.scaling.setAll(saved.scale);presentation.position.y=saved.lift;transition=saved.transition;camera.attachControl(canvas,true);cameraClearance.reset();gesture(saved.clip??'Idle');if(saved.width!==innerWidth||saved.height!==innerHeight)restoreView(active);};
 const stopWorldTour=()=>{cinematic=null;camera.attachControl(canvas,true);cameraClearance.reset();};
 const startWorldTour=(onDone:()=>void)=>{stopWorldTour();layout='tour';restoreView(-1);responsiveCamera();transition=0;camera.detachControl();cinematic={elapsed:0,onDone};gesture('Greet');};
 const updateFrame=()=>{
  ...
  if(!paused){
   ...
   handoffExhibit.update(dt,reduced);automationExhibit.update(dt,reduced);toolExhibit.update(dt,reduced);steeringExhibit.update(dt,reduced);reviewExhibit.update(dt,reduced);permissionExhibit.update(dt,reduced);modelExhibit.update(dt,reduced);contextExhibit.update(dt,reduced);browserExhibit.update(dt,reduced);branchExhibit.update(dt,reduced);harness.update(dt,reduced);const portraitLesson=innerWidth<=700&&active>=0&&layout==='mission';const targetScale=active<0?1.15:portraitLesson?.76:.86;const lift=portraitLesson?1.9:0;presentation.position.y=reduced?lift:presentation.position.y+(lift-presentation.position.y)*Math.min(1,dt*4);mascot.scaling.setAll(reduced?targetScale:mascot.scaling.x+(targetScale-mascot.scaling.x)*Math.min(1,dt*4));exhibitKey.position=new Vector3(mascot.position.x+2,6.6,mascot.position.z+.7);}
  if(depthCamera){if(paused){contextExhibit.update(0,true);browserExhibit.update(0,true);}const amount=reduced||paused?1:Math.min(1,dt*4);const destination=new Vector3(stations[depthCamera.station].position.x,2.9,-9.8);camera.setTarget(Vector3.Lerp(camera.target,destination,amount),false,true,true);camera.alpha+=(Math.PI/2-camera.alpha)*amount;camera.beta+=(1.36-camera.beta)*amount;camera.radius+=((innerWidth<=700?10:8)-camera.radius)*amount;mascot.position=Vector3.Lerp(mascot.position,new Vector3(stations[depthCamera.station].position.x-(depthCamera.station===9?2:2.6),depthCamera.station===9?.9:.85,depthCamera.station===9?-8.6:-9.2),amount);mascot.scaling.setAll(depthCamera.station===9?.52:.6);presentation.position.y=0;}
  if(cinematic&&!paused&&!document.hidden){
   cinematic.elapsed=Math.min(tourDuration,cinematic.elapsed+dt);const pose=sampleWorldTour(cinematic.elapsed,reduced);
   transition=0;active=pose.station;residency.focus(active);
   mascot.position.set(pose.x,.35,pose.z);mascot.scaling.setAll(1.15);presentation.position.y=0;
   const ending=Math.max(0,(cinematic.elapsed-41)/3);const e=ending*ending*(3-2*ending);
   camera.setTarget(new Vector3(pose.x-3.3*e,2.3,pose.z),false,true,true);
   camera.alpha=pose.alpha;camera.beta=pose.beta;camera.radius=pose.radius*(innerWidth<=700?.86:1);
   if(pose.moving&&clip?.name!=='Travel')gesture('Travel');else if(!pose.moving&&clip?.name==='Travel')gesture('Greet');
   if(pose.done){const done=cinematic.onDone;stopWorldTour();done();}
  }
 ...
 engine.runRenderLoop(()=>{
  ...
  if(residency.ready()){
   ...
   camera.update();
   updateFrame();
   if(!depthCamera&&!cinematic&&(transition===0||paused))cameraClearance.constrain();
  }
  residency.refresh();
