...
type Entry={definition:Definition;enabled:boolean;wanted:boolean;lastWanted:number;pending?:Promise<void>;controller?:Controller;factory?:any;owned?:Owned;args?:any[];snapshot:any;generation:number;attempt:number;loads:number;unloads:number;error:string|null};
...
export function createExhibitResidency(scene:Scene,camera:ArcRotateCamera,onStatus:(s:ExhibitStatus)=>void){
 ...
 function construct(e:Entry,factory:any,at:number){
  ...
  try{
   ...
   e.snapshot=controller.state();e.factory=factory;e.loads++;record('load-ready',e.definition.id,performance.now()-at);
  }catch(error){e.factory=undefined;e.controller=undefined;release(additions(scene,before));e.owned=undefined;throw error;}
 }
 function begin(e:Entry){
  ...
  const fail=(error:unknown)=>{if(!disposed&&generation===e.generation&&e.wanted&&e.enabled){e.error=String(error);record('load-failed',e.definition.id,performance.now()-at);}};
  // Resource eviction does not invalidate downloaded code. Reconstruct before
  // this frame's readiness check rather than yielding to a cached import promise.
  if(e.factory){record('factory-reuse',e.definition.id);try{construct(e,e.factory,at);}catch(error){fail(error);}return;}
  ...
  const job=Promise.race([module,new Promise<never>((_,reject)=>{timer=setTimeout(()=>reject(Error('Exhibit preparation timed out')),15000);})]).then(namespace=>{
   ...
   const factory=namespace[e.definition.exportName];if(typeof factory!=='function')throw Error('Exhibit factory unavailable');construct(e,factory,at);
  }).catch(fail).finally(()=>{clearTimeout(timer);if(e.pending===job)e.pending=undefined;});
  e.pending=job;
 }
 ...
 // Guided movement and camera input can change the view during scene.render,
 // after the outer readiness check. Recreate cached neighbours before Babylon
 // evaluates the actual camera's active meshes, including an instant jump.
 const cameraObserver=scene.onBeforeCameraRenderObservable.add(()=>{if(entries.some(e=>e.enabled&&e.factory&&!e.controller))refresh();});
 function dispose(){if(disposed)return;disposed=true;scene.onBeforeCameraRenderObservable.remove(cameraObserver);for(const e of entries){e.wanted=false;evict(e);e.factory=undefined;}onStatus({message:'',retry:false});}
 ...
 return {register,refresh,initial,ready,canRender,syncPresentation:()=>{for(const e of entries)e.controller?.update?.(0,true);},focus:(index:number)=>{active=index;},retry:()=>{for(const e of entries)if(e.enabled&&e.wanted&&e.error){e.error=null;e.attempt++;begin(e);}refresh();},
  state:()=>({active,ready:ready(),heldFrames,holdingSince:holdStart,events:[...events],entries:entries.map(e=>({id:e.definition.id,station:e.definition.station,enabled:e.enabled,wanted:e.wanted,loaded:!!e.controller,factoryCached:!!e.factory,pending:!!e.pending,loads:e.loads,unloads:e.unloads,error:e.error,resources:e.owned?{meshes:e.owned.meshes.length,materials:e.owned.materials.length,textures:e.owned.textures.length}:null}))}),
  dispose,
 };
}
