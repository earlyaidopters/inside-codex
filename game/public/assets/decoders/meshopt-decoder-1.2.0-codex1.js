// Local codex1 patch: retain worker blob URLs until every worker acknowledges loading; propagate worker failures and use the existing scalar decoder as recovery. Codec/WASM bytes are unchanged.
...
var MeshoptDecoder = (function () {
	...
	function createWorker(url, releaseUrl) {
		var released = false;
		...
		worker.object.onerror = function (event) {
			loaded();
	...
	function initWorkers(count) {
		var source =
			...
			';' +
			decode.toString() +
			workerProcess.toString() + ';self.postMessage({action:"worker-ready"});';
		...
		for (var i = workers.length; i < count; ++i) {
			workers[i] = createWorker(url, releaseUrl);
		}

		for (var i = count; i < workers.length; ++i) {
			workers[i].releaseUrl();
	...
	function decodeWorker(count, size, source, mode, filter) {
		...
		if (worker.failed) return Promise.reject(worker.failed);
	...
	return {
		...
		decodeGltfBufferAsync: function (count, size, source, mode, filter) {
			if (workers.length > 0) {
				return decodeWorker(count, size, source, decoders[mode], filters[filter]).catch(function () {
